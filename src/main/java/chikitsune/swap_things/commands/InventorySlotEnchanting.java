...
public class InventorySlotEnchanting {
 ...
 public static ArgumentBuilder<CommandSource, ?> register() { 
  return Commands.literal("inventoryslotenchanting").requires((cmd_init) -> { return cmd_init.hasPermissionLevel(Configs.cmdSTPermissionsLevel); }).executes((cmd_0arg) -> {
 ...
 private static int inventorySlotEnchantingLogic(CommandSource source,Collection<ServerPlayerEntity> targetPlayers, String slotNum, String fromName, Enchantment enchInput, Integer enchLevel) {
  ...
  for(ServerPlayerEntity targetedPlayer : targetPlayers) {
   ...
   if (tempStack.isEmpty()) {
    ArchCommand.playerMsger(source, targetPlayers, new StringTextComponent(TextFormatting.GOLD + "Well " + fromName + " wanted to be nice to " + TextFormatting.RED + targetedPlayer.getName().getString() + TextFormatting.GOLD + " and enchant one of their items but it was empty."));
    continue;
   }
